...
/// @arg speed
/// @arg obj
function MakeShape(argument0,argument1,argument2,argument3,argument4,argument5,argument6,argument7){
	var edge = argument0;
	var dx = argument1;
	var dy = argument2;
	...
	if(ang == -1)
		...
		ang = point_direction(dx,dy,PlayerGetX(),PlayerGetY()) + 180/argument5;
	...
	for(var i=0;i<=edge-1;i++)
	{
		...
		{
			inst[i][j] = instance_create_layer(dx,dy,dlayer,obj);
			inst[i][j].speed = dspd[j];
			inst[i][j].direction = ang + dir[j] + 360*i/edge  - ((edge-2)*180/edge/2);
